 ...
 *
 * Environment:
 *   TYPEFORGE_API_TOKEN - API token (default: reads from ~/.typeforge-token)
...
function getToken() {
  if (process.env.TYPEFORGE_API_TOKEN) return process.env.TYPEFORGE_API_TOKEN;
  ...
  console.error('Error: No API token found. Set TYPEFORGE_API_TOKEN env var or create ~/.typeforge-token file.');
  process.exit(1);
}
...
Token: Set TYPEFORGE_API_TOKEN env var or create ~/.typeforge-token file`);
}
