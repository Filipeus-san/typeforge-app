    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
          <button className="btn-outline-tf btn-sm" onClick={onCancel} style={{ padding: '0.5rem 1.5rem', borderRadius: 8 }}>Zrušit</button>
          <button className="btn-primary-tf btn-sm" onClick={onConfirm} style={{ padding: '0.5rem 1.5rem', borderRadius: 8 }}>Potvrdit</button>
        </div>
      </div>
    </div>
  );
