...
Wrapper script for the TypeForge MCP API. Replaces direct MCP tool calls. Token is stored in `~/.typeforge-token` (home directory, outside the repo).
...
- **To verify migration ran:** Check logs or use `node typeforge-mcp.js run_sql_query project_id=5 environment_id=7 query="SELECT * FROM _migrations"` to check
- **If migration didn't run:** Either wait for cold start, or manually run the migration SQL using `typeforge-mcp.js run_sql_query`, then insert record into `_migrations` table:
  ```sql
