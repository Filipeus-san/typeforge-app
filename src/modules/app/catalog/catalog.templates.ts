            ...
            <div class="row g-4">
                <div class="col-md-8">
                    ${CardSection({
                        ...
                        children: `
                            <div class="row g-3">
                                <div class="col-md-6">${FormGroup({ label: CATALOG_T.form.labels.categoryName, required: true, children: Input({ name: 'name', value: data?.name ?? '', required: true }) })}</div>
                                <div class="col-md-6">${FormGroup({ label: CATALOG_T.form.labels.slug, children: Input({ name: 'slug', value: data?.slug ?? '', placeholder: CATALOG_T.form.placeholders.autoFromName }) })}</div>
                            </div>
                            <div class="mb-3">${FormGroup({ label: CATALOG_T.form.labels.description, children: Textarea({ name: 'description', value: data?.description ?? '', rows: 3 }) })}</div>
                        `
                    })}
                </div>
                <div class="col-md-4">
                    ...
                    ${CardSection({
                        ...
                        children: `
                            <div class="mb-3">${FormGroup({ label: CATALOG_T.form.labels.icon, children: Input({ name: 'icon', value: data?.icon ?? 'tag', placeholder: CATALOG_T.form.placeholders.categoryIconExamples }) })}</div>
                            <div class="mb-3">${FormGroup({ label: CATALOG_T.form.labels.status, children: Select({ name: 'status', options: statusOptions }) })}</div>
                            <div class="mb-3">${FormGroup({ label: CATALOG_T.form.labels.sortOrder, children: `<input type="number" name="sort_order" class="form-control" value="${escapeHtml(data?.sort_order ?? '0')}" min="0">` })}</div>
                        `
                    })}
                    ${CardSection({
                        children: `
                            <div class="d-grid gap-2">
                                <button type="submit" class="btn-add w-100 justify-content-center">
                                    ${Icon({ name: isEdit ? 'check-lg' : 'plus-lg' })}
                                    ${isEdit ? CATALOG_T.actions.saveChanges : CATALOG_T.actions.createCategory}
                                </button>
                                <a href="/admin/categories" class="btn btn-outline-tf btn-sm text-center">Zpět</a>
                            </div>
                        `
                    })}
                </div>
