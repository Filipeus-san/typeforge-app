...
export function insertCategory(name: string, slug: string, description: string, icon: string, status: string, sortOrder: number): void {
    sqlQuery<DbCategory>(
        `INSERT INTO categories (name, slug, description, icon, status, sort_order)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [name, slug, description, icon, status, sortOrder]
    );
}

export function updateCategory(id: number, name: string, slug: string, description: string, icon: string, status: string, sortOrder: number): void {
    sqlQuery<DbCategory>(
        `UPDATE categories SET name = $1, slug = $2, description = $3, icon = $4, status = $5, sort_order = $6, updated_at = CURRENT_TIMESTAMP WHERE id = $7`,
        [name, slug, description, icon, status, sortOrder, id]
    );
}
