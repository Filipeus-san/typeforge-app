...
import { getCurrentUserInfo } from "./modules/app/shared";
...
export function getReactPageTemplate(
    ...
    // Inject current user info for sidebar avatar
    let currentUserScript = '';
    const userInfo = getCurrentUserInfo();
    if (userInfo !== null) {
        const userJson = safePropsJson({ name: userInfo.name, avatarUrl: userInfo.avatarUrl });
        currentUserScript = `\n    <script>window.__CURRENT_USER__=${userJson};</script>`;
    }

    // Inject language for React i18n
    const langScript = `\n    <script>window.__LANG__="${getCurrentLanguage()}";</script>`;

    return `<!DOCTYPE html>
<html lang="${getCurrentLanguage()}" data-tf-theme="dark">
<head>
    <meta charset="UTF-8" />
...
<body>${currentUserScript}${langScript}
